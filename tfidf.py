...
def segFile(segFileName, recordPath = 'segFile', filePath = 'corpus'):
    ...
    # 对文档进行分词处理，采用默认模式
    segFileData = pseg.cut(fileData)

    # 对空格，换行符进行处理
    result = []
    for line in segFileData:
...
def Tfidf(segFileNameList, count = 10, segFilePath = 'segFile'):
    ...
    for i in range(len(weight)):
        print u"--------Writing all the tf-idf in the", i, u" file --------"
...
if __name__ == "__main__":
    ...
    print keywordDatas
    print json.dumps(keywordDatas.get('1000005'), ensure_ascii=False)